...
4. **Composer clearance:** coach composer and message list stop reading `BottomTabBarHeightContext`; they clear the capsule via `insets.bottom + FLOATING_TAB_BAR_CLEARANCE` from `src/navigation/tabBarMetrics.ts`, matching `Screen`'s fixed formula. *Errata:* the landed behavior (plan task 3) is that the message list does NOT get extra clearance, because the composer dock's in-flow padding already lifts it — duplicating 110pt there would be dead space.
