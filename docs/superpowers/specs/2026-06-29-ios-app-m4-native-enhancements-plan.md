...
4. **映射 + 上报**：把 HealthKit 样本映射为 `POST /api/v1/sync/coros`（纳入 v1 的 sync 端点）可接受的 payload 形态，`source="healthkit"`：
   ...
   - sleep analysis → `SleepRecord`
   - HRV/restingHR → `RecoveryRecord`
...
3. **提醒调度**：训练提醒可由服务端定时任务（见 T3 的服务端侧）按用户时区在任务前 X 分钟推送。
...
2. **服务端定时**：COROS/菜单同步与训练提醒由服务端 cron（部署平台的 scheduled job）按用户时区触发，不依赖 App 在前台。
3. **节流**：尊重 iOS 后台执行预算，失败重试与退避。
...
4. **prebuild vs 继续 managed + config plugin**：默认 prebuild，工程更可控。
